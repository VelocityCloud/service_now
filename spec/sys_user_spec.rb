  ...
  describe '.find' do
    it 'finds a record by the sys id' do
      VCR.use_cassette('sys_user') do
        expect(ServiceNow::SysUser.find('000d673968d3b10001fbb0343cff4e12', auth)['result']['first_name']).to eql('Frank')
      end
    end
  end
  ...
  describe '.update' do
    it 'updates a record' do
      VCR.use_cassette('update_sys_user') do
        ...
        expect(ServiceNow::SysUser.update('62d0a6100a0a3c5400d13bd0d4409f54', params, auth)['result']['last_name']).to eql('Miller')
      end
    end
  end
end
