  ...
  describe '.all' do
    it 'finds records by the params' do
      VCR.use_cassette('sys_users') do
        sys_users = ServiceNow::SysUser.all({ sysparm_limit: 25 }, auth)
  ...
  describe '.find' do
    it 'finds a record by the sys id' do
      VCR.use_cassette('sys_user') do
        expect(ServiceNow::SysUser.find('000d673968d3b10001fbb0343cff4e12', auth)['first_name']).to eql('Frank')
      end
    end
  end
  ...
  describe '.create' do
    xit 'creates a record' do
      VCR.use_cassette('create_sys_user') do
        ...
        expect(ServiceNow::SysUser.create(params, auth)['sys_id']).to eql('9719345913621ec0fef07d322244b058')
      end
    end
  end
  ...
  describe '.update' do
    it 'updates a record' do
      VCR.use_cassette('update_sys_user') do
        ...
        expect(ServiceNow::SysUser.update('62d0a6100a0a3c5400d13bd0d4409f54', params, auth)['last_name']).to eql('Miller')
      end
    end
  end
end
